...
import com.espressif.iot.esptouch.udp.LogUtils;
...
public class __EsptouchTask implements __IEsptouchTask {
	...
	public __EsptouchTask(String apSsid, String apBssid, String apPassword,IEsptouchTaskParameter parameter,
			boolean isSsidHidden) {
		LogUtils.debug("Welcome Esptouch " + ESPTOUCH_VERSION);
		if (apPassword == null) {
			apPassword = "";
		}
		mApSsid = apSsid;
	...
	private void __putEsptouchResult(boolean isSuc, String bssid,
		...
		synchronized (mEsptouchResultList) {
			...
			if (__IEsptouchTask.DEBUG) {
				LogUtils.debug("__putEsptouchResult(): count = " + count);
			}
			...
			if (!isExist) {
				if (__IEsptouchTask.DEBUG) {
					LogUtils.debug( "__putEsptouchResult(): put one more result");
				}
	...
	private void __listenAsyn(final int expectDataLen) {
		mTask = new Thread() {
			public void run() {
				if (__IEsptouchTask.DEBUG) {
					LogUtils.debug( "__listenAsyn() start");
				}
				...
				if (__IEsptouchTask.DEBUG) {
					LogUtils.debug("expectOneByte: " + (0 + expectOneByte));
				}
				...
				while (mEsptouchResultList.size() < mParameter
					...
					if (receiveOneByte == expectOneByte) {
						if (__IEsptouchTask.DEBUG) {
							LogUtils.debug("receive correct broadcast");
						}
						...
						if (timeout < 0) {
							if (__IEsptouchTask.DEBUG) {
								LogUtils.debug( "esptouch timeout");
							}
							break;
						} else {
							if (__IEsptouchTask.DEBUG) {
								LogUtils.debug( "mSocketServer's new timeout is "
						...
						if (__IEsptouchTask.DEBUG) {
							LogUtils.debug( "receive rubbish message, just ignore");
						}
					}
				}
				...
				if (__IEsptouchTask.DEBUG) {
					LogUtils.debug( "__listenAsyn() finish");
				}
			}
		};
		mTask.start();
	...
	public List<IEsptouchResult> executeForResults(int expectTaskResultCount)
		...
		if (__IEsptouchTask.DEBUG) {
			LogUtils.debug("execute()");
		}
		InetAddress localInetAddress = EspNetUtil.getLocalInetAddress();
		if (__IEsptouchTask.DEBUG) {
			LogUtils.debug( "localInetAddress: " + localInetAddress);
		}
